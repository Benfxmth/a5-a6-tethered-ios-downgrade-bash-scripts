...
asr restore -source /var/UDZO.dmg -target /dev/disk0s1s1 -erase -noprompt -puppetstrings && fsck_hfs -fy /dev/disk0s1s1 && mount -t hfs /dev/disk0s1s1 /mnt1 && fsck_hfs -fy /dev/disk0s1s2 && mount -t hfs /dev/disk0s1s2 /mnt2 && cd /usr/local/standalone/firmware/Baseband/Mav7Mav8/ && mkdir -p /mnt1/usr/local/standalone/firmware/Baseband/Mav7Mav8 && cp -a /var/Mav7Mav8/* /mnt1/usr/local/standalone/firmware/Baseband/Mav7Mav8 && mv -v /mnt1/private/var/* /mnt2 && cp -a /var/fstab /mnt1/etc && umount /mnt2 && mount -t hfs /dev/disk0s1s2 /var && fixkeybag && umount -f /var && mount -t hfs /dev/disk0s1s2 /mnt2 && mv /apticket.der /mnt1/System/Library/Caches && mv /kernelcache /mnt1/System/Library/Caches/com.apple.kernelcaches && echo Cleaning up... && rm -rf /var/UDZO.dmg &&
