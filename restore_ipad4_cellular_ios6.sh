...
asr restore -source /var/UDZO.dmg -target /dev/disk0s1s1 -erase -noprompt -puppetstrings && fsck_hfs -f /dev/disk0s1s1 && mount -t hfs /dev/disk0s1s1 /mnt1 && fsck_hfs -f /dev/disk0s1s2 && mount -t hfs /dev/disk0s1s2 /mnt2 && cd /usr/local/standalone/firmware/Baseband/Mav5/ && mkdir -p /mnt1/usr/local/standalone/firmware/Baseband/Mav5 && cp -a /usr/local/standalone/firmware/Baseband/Mav5/Mav5-personalized.zip /mnt1/usr/local/standalone/firmware/Baseband/Mav5 && mv -v /mnt1/var/* /mnt2 && cp -a /var/fstab /mnt1/etc && umount /mnt2 && mount -t hfs /dev/disk0s1s2 /var && fixkeybag && umount -f /var && mount -t hfs /dev/disk0s1s2 /mnt2 && mv /kernelcache /mnt1/System/Library/Caches/com.apple.kernelcaches && rm -rf /var/UDZO.dmg &&
